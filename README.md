...
### Usage
Both class-based and Express middlewares can be used at different levels, executed by the following priorities:

```ts
// At application level (in config)
yasui.createServer({
    middlewares: [HelloMiddleware, myMiddleware()]
});

// At controller level
@Controller('/', HelloMiddleware, myMiddleware())
export class MyController { /* ... */ }

// At endpoint level
@Get('/:name', HelloMiddleware, authMiddleware)
private myEndpoint() { /* ... */ }
```
